    ...
    try {
      ...
      if (txn.type === "payment") {
        ...
        if (txn.source === "vendor_purchase") {
          ...
          // Filter payments that match this purchase - check multiple patterns
          const matchingPayments = relatedPayments?.filter(p => {
            if (!p.notes) return false;
            const notes = p.notes.toLowerCase();
            // Match by purchase ID in various note formats
            return notes.includes(`purchase ${txn.id}`) || 
                   notes.includes(`purchase id: ${txn.id}`) ||
                   notes.includes(`vendor_purchase ${txn.id}`) ||
                   (notes.includes('auto') && p.payment_date === txn.date && Math.abs(p.amount - txn.amount) < 0.01);
          }) || [];
          ...
          const matchingPayments = relatedPayments?.filter(p => {
            ...
            return notes.includes(`purchase ${txn.id}`) || 
                   notes.includes(`purchase id: ${txn.id}`) ||
                   notes.includes(`material_purchase ${txn.id}`) ||
                   (notes.includes('auto') && p.payment_date === txn.date && Math.abs(p.amount - txn.amount) < 0.01);
          }) || [];
                  ...
                  {transactions.length === 0 ? (
                    ...
                    (() => {
                      ...
                      return transactions.map((txn) => {
                        ...
                        // For purchases, check if they were paid immediately (cash/bank/mfs)
                        if (txn.type === "purchase") {
                          // If payment method is cash, bank, or mfs, it's already paid (no balance change)
                          if (txn.payment_method === "cash" || txn.payment_method === "bank" || txn.payment_method === "mfs") {
                            // Purchase was paid immediately, so no due amount - balance stays same
                          } else {
